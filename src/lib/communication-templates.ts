import type { Claim, CommunicationRecipient, DraftCommunication, WorkflowState } from '@/types'
...
// Each template returns partial draft fields (no id/claimId/createdAt/recipient)

type TemplateDraft = { trigger: string; to: string; subject: string; body: string }
...
function makeDraft(claim: Claim, draft: TemplateDraft, recipient: CommunicationRecipient): DraftCommunication {
  return {
    ...draft,
    id: nextId(),
    claimId: claim.id,
...
export function generateCommunication(claim: Claim, toState: WorkflowState): DraftCommunication[] {
  ...
  if (triggers.insured) {
    ...
    if (generator) {
      ...
      drafts.push(makeDraft(claim, insuredDraft, 'insured'))

      // Auto-generate broker copy
      const brokerDraft = generateBrokerCopy(claim, insuredDraft)
      if (brokerDraft) {
        drafts.push(makeDraft(claim, brokerDraft, 'broker'))
      }
    }
  }
  ...
  if (triggers.provider) {
    ...
    if (generator) {
      drafts.push(makeDraft(claim, generator(claim), 'provider'))
    }
  }
...
export function generateFromTemplate(claim: Claim, templateKey: string): DraftCommunication | null {
  ...
  const recipient: CommunicationRecipient = insuredTemplates[templateKey] ? 'insured' : 'provider'
  return makeDraft(claim, draft, recipient)
}
