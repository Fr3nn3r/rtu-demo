...
export function Closed({ claim }: { claim: Claim }) {
  ...
  let icon = <CheckCircle className="size-6 text-primary" />
  ...
  if (wasRejected) {
    ...
    icon = <AlertTriangle className="size-6 text-accent-foreground" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div>
          <div className="text-[11px] font-medium text-muted-foreground uppercase">Created</div>
          <div>{format(new Date(claim.createdAt), 'dd MMM yyyy, HH:mm')}</div>
        </div>
        <div>
          <div className="text-[11px] font-medium text-muted-foreground uppercase">Closed</div>
